...
Individual *roulette(Individual **population, int p_size){
  ...
  return individual_clone(population[i],0);
}

void getElite(Individual **population, int p_size, Individual **elite, int e_size){
  int i;
  for (i = 0; i < e_size; i++) {
    elite[i] = individual_clone(population[i],0);
  }
}
...
void STMUT(Individual *parent){
  ...
  for(i = 0; i < parent->n_weights; i++){
    isFlip = getRandomDouble() < (1/(double)parent->n_weights);
    if (isFlip){
      parent->weights[i] += (getRandomDouble() - getRandomDouble())*0.1;
